...
RegisterNetEvent("sjr_me:sound")
AddEventHandler("sjr_me:sound", function(url, volume, x, y, z, max_dist)
    local playerPed = PlayerPedId()  -- Obtém o ped do jogador
    local playerCoords = GetEntityCoords(playerPed)  -- Obtém as coordenadas do jogador
    
    -- Calcula a distância entre o jogador e as coordenadas fornecidas
...
-- Evento para tocar o som no cliente
RegisterNetEvent("playSound")
AddEventHandler("playSound", function(url, volume, soundCoords, max_dist)
    local playerPed = PlayerPedId() -- Obtém o ped do jogador
    local playerCoords = GetEntityCoords(playerPed) -- Obtém as coordenadas do jogador

    -- Calcula a distância entre o jogador e as coordenadas do som
    local distance = Vdist(playerCoords.x, playerCoords.y, playerCoords.z, table.unpack(soundCoords))

    -- Verifica se a distância é menor ou igual à distância máxima
    if distance <= max_dist then
        SendNUIMessage({act = "play_audio_source", url = url, x = soundCoords[1], y = soundCoords[2], z = soundCoords[3], volume = volume, max_dist = max_dist})
    end
end)


RegisterNetEvent("sjr_me:checkAndPlaySound")
AddEventHandler("sjr_me:checkAndPlaySound", function(url, volume, soundCoords, max_dist)
    local playerPed = PlayerPedId() -- Obtém o ped do jogador
    local playerCoords = GetEntityCoords(playerPed) -- Obtém as coordenadas do jogador
    ...
    -- Verifica se a distância é menor ou igual à distância máxima
    if distance <= max_dist then
        -- Toca o som se o jogador estiver dentro da área
        SendNUIMessage({act = "play_audio_source", url = url, x = soundCoords[1], y = soundCoords[2], z = soundCoords[3], volume = volume, max_dist = max_dist})
    end
end)
