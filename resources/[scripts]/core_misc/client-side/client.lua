...
local cache = {}
function getId(plate)
	if not cache[plate] then 
		cache[plate] = vSERVER.getId(plate)
	end
	return cache[plate]
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
local polmav_hash = GetHashKey("odv-heli")
...
AddEventHandler('gameEventTriggered', function(eventName, eventArgs)
	if not (eventName == 'CEventNetworkPlayerEnteredVehicle') then return end
	Wait(100)
	StartPolmavThread()
end)

function StartPolmavThread()
	if threadStarted then return end
	threadStarted = true
	while IsPlayerInPolmav() do
