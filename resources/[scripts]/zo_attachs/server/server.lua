...
-- Inicializa a tabela caso não esteja inicializada
cacheAttachsPlayers = cacheAttachsPlayers or {}
...
AddEventHandler(
    ...
    function(reason)
        ...
        -- Verificação se user_id é nil
        if not user_id then
            print("Error: user_id is nil for source:", source)
            return -- Sai da função se user_id for nil
        end

        -- Remover o user_id do cache se existir
