...
local twoPed = {
	{
		event = "vrp_policia:algemar",
		label = "Algemar",
		tunnel = "server"
	},
	{
		event = "item:usarCapuz",
		label = "Capuz",
		tunnel = "server"
	},
	{
...
local adminMenu = {
	{
		event = "bm_module:deleteVehicles",
					...
					elseif IsPedAPlayer(entity) then
						if #(coords - entCoords) <= 1.0 then
							...
							if vSERVER.PermissionParamedico() then
								...
								local like, deslike = vSERVER.getLikeInfos(source)
								twoPed[6].label = "Dar like ("..like..") 👍"
								twoPed[7].label = "Dar Deslike ("..deslike..") 👎"
