...
creatorCoords = vec3(-582.84,50.67,87.43) -- vec3(-1062.02,-2711.85,0.83) -- -1670.72,-917.93,8.83,345.83
creatorHeading = 90.71


spawnCoords = vec3(-2092.45,-540.38,12.62)
spawnHeading = 102.05
...
function initCreator()
    loadingPlayer(true)
    TriggerEvent('vrp_hud:show', false)
...
function setGender(gender)
    -- Definir o modelo padrão como masculino
    ...
    elseif gender ~= "male" then
        model = gender  -- Permitir outros modelos customizados
    end

    -- Requisitar e carregar o modelo
    RequestModel(model)
    while not HasModelLoaded(model) do
        Citizen.Wait(100)
    end

    -- Obter o ped atual do jogador
    local ped = PlayerPedId()
    
    -- Obter valores atuais de vida, vida máxima e armadura
    local currentHealth = GetEntityHealth(ped)
    local currentMaxHealth = GetPedMaxHealth(ped)
    local currentArmour = GetPedArmour(ped)

    -- Mudar o modelo do jogador
    SetPlayerModel(PlayerId(), model)
    
    -- Atualizar a referência do ped
    ped = PlayerPedId()

    -- Aplicar valores antigos de vida, vida máxima e armadura
    SetPedMaxHealth(ped, currentMaxHealth)  -- Definir vida máxima baseada no ped anterior
    SetEntityHealth(ped, currentHealth)     -- Restaurar vida atual
    SetPedArmour(ped, currentArmour)        -- Restaurar armadura atual

    -- Resetar as roupas do jogador (função customizada)
    resetCloths()

    -- Aplicar a cor da cabeça de acordo com o gênero
    if gender == "male" then
        SetPedHeadBlendData(ped, 21, 0, 0, 21, 0, 0, 0.8, 0.8, 0.0, false)  -- Para masculino
    else
        SetPedHeadBlendData(ped, 21, 0, 0, 21, 0, 0, 0.2, 0.2, 0.0, false)  -- Para feminino
    end
end
...
function cutScene() -- ORDER CREATION
	...
	NewLoadSceneStartSphere(-582.84,50.67,87.43, 1000, 0) ----- avoid texture bugs
    -----------------------------------------------
    inCutscene = true
