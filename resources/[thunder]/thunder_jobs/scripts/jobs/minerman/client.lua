		...
		if not working then
			...
			if not IsPedInAnyVehicle(ped) then
				...
				if distance <= 100.0 then
					idle = 5
					DrawMarker(21, rocks[selected].x, rocks[selected].y, rocks[selected].z-0.3, 0, 0, 0, 0, 180.0, 130.0, 0.6, 0.8, 0.5, 255, 0, 0, 180, 1, 0, 0, 1)
					if distance <= 1.2 and IsControlJustPressed(1,38) then
						--if lastVehicle == GetHashKey('TipTruck') then
							--emP.addBlips2()
							...
							--else
							--	TriggerEvent("Notify","negado","<b>espaço na mochila</b> insuficientes.")
							--end
						--[[ else
