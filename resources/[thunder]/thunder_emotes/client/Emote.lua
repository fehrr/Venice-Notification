...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/emotemenuwc', 'Open rpemotes menu (F4) by default. This may differ from server to server.')
...
RegisterCommand('u', function(source, args, raw) EmoteCommandStart(source, args, raw) end, false)
RegisterCommand('emote2', function(source, args, raw) EmoteCommandStart(source, args, raw) end, false)
if Config.SqlKeybinding then
...
function EmoteCommandStart(source, args, raw) 
    if #args > 0 then
        ...
        -- Removida a verificação de emotes válidos
        if RP.Emotes[name] ~= nil then
            OnEmotePlay(RP.Emotes[name], name)
            return
        ...
        -- Removida a mensagem de emote não válido
