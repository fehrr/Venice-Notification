class Inventory {
    ...
    getItemHtml(item, target, slot, method = "allDiv") {
        if (method == "onlyItems") {
            return `
                ...
                <img src="http://181.215.254.182/inventario/${item.item}.png" onerror="this.src='../assets/images/no_image.png'"/>
                <p>${item.name}</p>
            `
        }
        return `
    ...
    selectItem(side, itemId) {
        ...
        $('.image-grame').attr('src', `http://181.215.254.182/inventario/${globalThis.SelectedItem.item}.png`);
