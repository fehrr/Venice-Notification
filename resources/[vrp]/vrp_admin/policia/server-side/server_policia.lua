...
AddEventHandler('atirando',function(x,y,z)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if not pickTime[user_id] or pickTime[user_id] <= os.time() then
			pickTime[user_id] = os.time()+30
			if not vRP.hasPermission(user_id,"perm.disparo") then
				...
				for k,v in pairs(policia) do
					...
					if player then
						async(function()
							TriggerClientEvent("NotifyPush",player,{ time = os.date("%H:%M:%S - %d/%m/%Y"), code = 10, title = "Confronto em andamento", x = x, y = y, z = z, criminal = "Disparos de arma de fogo", rgba = {105,52,136} })
						end)
					end
				end
			end
