...
RegisterCommand('rgbcar',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	 if user_id then
		 if vRP.hasPermission(user_id,"admin.permissao") then -- permissão entre "Admin"
...
RegisterCommand("mefox", function(source, args, rawCommand)
    local user_id = vRP.getUserId(source)
    if user_id then
        local emoji = vRP.prompt(source, "Digite o emoji que deseja usar (máx. 5 caracteres) (ex: 👍, 👎, ❤):", "")
        if emoji == "" then
            emoji = "👍" 
        elseif #emoji > 5 then
            TriggerClientEvent("Notify", source, "negado", "O emoji pode ter no máximo 5 caracteres.", 6000)
            return 
        end
...
RegisterCommand("tirarme", function(source, args, rawCommand)
    local user_id = vRP.getUserId(source)
    if user_id then
        local playerSource = source
        local nplayers = vRPclient.getNearestPlayers(source, 10)
        for k in pairs(nplayers) do
            TriggerClientEvent("vrp_player:removeMeText", parseInt(k), playerSource)
        end
        TriggerClientEvent("vrp_player:removeMeText", source, playerSource)
    ...
    if source then
        exports.oxmysql:execute("SELECT texto, emoji FROM foxzin_texto WHERE user_id = @user_id LIMIT 1", {
            ...
            if result[1] then
                ...
                TriggerClientEvent("vrp_player:showMeText", -1, source, text, emoji, {10, 250, 0, 255})
            else
