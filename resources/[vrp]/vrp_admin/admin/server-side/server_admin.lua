...
--                 end
--             end
--             os.execute("start C:/Users/Administrador/Desktop/thunderRJ-Group/start.bat +exec server.cfg")
...
local cache = {}
local cacheTime = 60 * 1000 -- Tempo em milissegundos para armazenar o cache

-- Função para obter dados do cache
local function getFromCache(key)
    if cache[key] and (os.time() * 1000 - cache[key].time < cacheTime) then
...
local function addToCache(key, data)
    cache[key] = { data = data, time = os.time() * 1000 }
end

-- Função para carregar streams automaticamente
local function loadStreamAutomatically(playerId)
    local streams = {"foxzin-maps", "foxzin-vehicles", "foxzin_roupas"} -- Substitua pelos nomes reais dos streams

    for _, streamName in ipairs(streams) do
        local cachedStream = getFromCache(streamName)
        if cachedStream then
            -- Enviar stream do cache
            TriggerClientEvent("vRP:sendStream", playerId, cachedStream)
        else
            -- Carregar stream normalmente e adicionar ao cache
            local streamData = "Dados do stream para " .. streamName -- Simulação de dados do stream
            addToCache(streamName, streamData)
            TriggerClientEvent("vRP:sendStream", playerId, streamData)
        end
    end
end

-- Evento que é chamado quando um jogador se conecta
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        loadStreamAutomatically(source)
    end
end)
